<?php
namespace um_ext\um_user_tags\admin\core;

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) exit;

if ( ! class_exists( 'um_ext\um_user_tags\admin\core\Admin_Upgrade' ) ) {
    /**
    ...
    class Admin_Upgrade {
        ...
        function __construct() {
            ...
            $um_user_tags_last_version_upgrade = get_option( 'um_user_tags_last_version_upgrade' );

            if ( ! $um_user_tags_last_version_upgrade || version_compare( $um_user_tags_last_version_upgrade, um_user_tags_version, '<' ) )
        ...
        public function packages() {
            ...
            $um_user_tags_last_version_upgrade = get_option( 'um_user_tags_last_version_upgrade' );
            $um_user_tags_last_version_upgrade = ! $um_user_tags_last_version_upgrade ? '0.0.0' : $um_user_tags_last_version_upgrade;

            foreach ( $this->update_versions as $update_version ) {

                if ( version_compare( $update_version, $um_user_tags_last_version_upgrade, '<=' ) )
                    continue;

                if ( version_compare( $update_version, um_user_tags_version, '>' ) )
                    continue;
                ...
                if ( file_exists( $file_path ) ) {
                    ...
                    update_option( 'um_user_tags_last_version_upgrade', $update_version );
                }
            }
