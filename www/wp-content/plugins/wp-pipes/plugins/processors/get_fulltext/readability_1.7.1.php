...
class Readability {
	...
	protected function getArticleTitle() {
		...
		if ( preg_match( '/ [\|\-] /', $curTitle ) ) {
			...
			if ( strpos( $curTitle, ': ' ) !== false ) {
				...
				if ( strlen( $curTitle ) > 150 || strlen( $curTitle ) < 15 ) {
	 ...
	 * @return void
	 */
	public static function revertReadabilityStyledElements( $articleContent ) {
	 ...
	 * @return void
	 */
	public static function prepArticle( $articleContent ) {
						...
						if ( ( $img > $p ) && ( $img > 4 ) ) {
							...
							if ( $li > $p && $tag != 'ul' && $tag != 'ol' ) {
								...
								if ( $input > floor( $p / 3 ) ) {
									...
									if ( $contentLength < 25 && ( $embedCount === 0 && ( $img === 0 || $img > 2 ) ) ) {
