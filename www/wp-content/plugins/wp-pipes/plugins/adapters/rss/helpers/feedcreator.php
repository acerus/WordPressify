<?php
/***************************************************************************
 * FeedCreator class v1.8.1 (unofficial)
 ...
 * v1.7.4 Custom XML by Thong Tran (thongta@foobla.com)
 * v1.8.1 merged w/ 1.8.0-dev by Thong Tran (thongta@foobla.com)
 ...
 * Changelog:
 * 1.8.2
 * 23-June-2012 Thong Tran
 * added Joomla FTP layer support
 * 1.8.1
 * 24-Feb-2012 Thong Tran
 * merged w/ 1.8.0-dev version
 * 1.7.4
 * 21-Sep-2011 Thong Tran
 * added Custom Elements support for creating RSS Feed
 * fixed Improved XML layout
 * 1.7.3    10-11-04
 ...
 * fixed author/creator in RSS10 (thanks Tanguy Pruvot)
 * v1.6 beta    02-28-04
 ...
 * echo $rss->saveFeed("RSS10", "news/feed.xml");
 * //to generate "on-the-fly"
 * $rss->outputFeed("RSS10");
 ***************************************************************************
 *          A little setup                                                 *
 **************************************************************************/
 ...
 * Version string.
 **/
define( "FEEDCREATOR_VERSION", "FeedCreator 1.8.3" );
define( "GENERATOR", "obRSS 1.9" );
/**
...
class UniversalFeedCreator extends FeedCreator {
	...
	function _setFormat( $format ) {
		switch ( strtoupper( $format ) ) {
			...
			case "JAVASCRIPT":
				...
				break;
			case "JSON":
				$this->_feed = new OBJSONCreator();
				break;
			case "SITEMAP":
				$this->_feed = new GoogleSiteMapIndex();
				break;
			default:
				$this->_feed = new RSSCreator091();
				break;
	 ...
	 *                     "PIE01", "mbox", "RSS091", "RSS10", "RSS20", "OPML", "ATOM03", "HTML", "JS"
	 *
	 * @return    string    the contents of the feed.
	 */
	function createFeed( $format = "RSS091" ) {
	 ...
	 *
	 * @param    string     format    format the feed should comply to. Valid values are:
	 *                      "PIE01" (deprecated), "mbox", "RSS091", "RSS10", "RSS20", "OPML", "ATOM", "ATOM03", "HTML", "JS"
	 * @param    string     filename    optional    the filename where a recent version of the feed is saved. If not specified, the filename is $_SERVER["PHP_SELF"] with the extension changed to .xml (see _generateFilename()).
	 * @param    boolean    displayContents    optional    send the content of the file or not. If true, the file will be sent in the body of the response.
	 */
	function saveFeed( $format = "RSS091", $filename = "", $displayContents = true ) {
		$this->_setFormat( $format );
	 ...
	 *
	 * @param    string      format    format the feed should comply to. Valid values are:
	 *                       "PIE01" (deprecated), "mbox", "RSS091", "RSS10", "RSS20", "OPML", "ATOM03".
	 * @param    filename    string    optional the filename where a recent version of the feed is saved. If not specified, the filename is $_SERVER["PHP_SELF"] with the extension changed to .xml (see _generateFilename()).
	 * @param    timeout     int      optional the timeout in seconds before a cached version is refreshed (defaults to 3600 = 1 hour)
	 */
	function useCached( $format = "RSS091", $filename = "", $timeout = 3600 ) {
		$this->_setFormat( $format );
	 ...
	 *                            "PIE01" (deprecated), "mbox", "RSS091", "RSS10", "RSS20", "OPML", "ATOM03".
	 */
	function outputFeed( $format = 'RSS091' ) {
		$this->_setFormat( $format );
...
class FeedDate {
	 ...
	 * $tzOffset = "1"; // offset for time (hours)
	...
	function __construct( $dateString = "" ) {
		if ( $dateString == '' ) {
			...
			$daylight_saving = date( 'I', $dateString ); // Whether or not the date is in daylight saving time
		}
	...
	function createFeed() {
		$feed = '
		{
			...
			"value":
				{
					"feedUrl":"' . $this->link . '",
					"title":"' . FeedCreator::iTrunc( htmlspecialchars( $this->title ), $this->truncSize ) . '",
					"link":"' . $this->link . '",
					"author":"",
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			$feed .= '
							{
								"title":"' . FeedCreator::iTrunc( htmlspecialchars( strip_tags( $this->items[$i]->title ) ), $this->truncSize ) . '",
			';
			$feed .= '
								"link":"' . htmlspecialchars( $this->items[$i]->link ) . '",
			';
			// date
			if ( $this->items[$i]->date != "" ) {
				$itemDate = new FeedDate( $this->items[$i]->date );
				$feed .= '
								"pubDate":"' . htmlspecialchars( $itemDate->rfc822() ) . '",
				';
			}
			// contentSnippet - A snippet (< 120 characters) version of the content attribute. The snippet does not contain any HTML tags.
			$item_description = $this->items[$i]->getDescription();
			$contentSnippet   = FeedCreator::iTrunc( htmlspecialchars( strip_tags( $item_description ) ), 120 );
			$contentSnippet   = $this->validateJSONFeed( $contentSnippet );
// 			$feed .= '
// 								"description":'.$contentSnippet.',
// 			';
			$feed .= '
								"description":' . $this->validateJSONFeed( $item_description ) . '
			';
			$categories = $this->items[$i]->category;
			if ( $categories != NULL ) {
				$feed .= ',
								"categories":[
				';
				...
				foreach ( $categories AS $category ) {
					...
					$feed .= '
						' . $this->validateJSONFeed( $category ) . '
					';
					$j ++;
				}
		...
		';
		// remove all tabs and new line characters
	...
	function createFeed_bak() {
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			if ( $this->items[$i]->date != "" ) {
				$itemDate = new FeedDate( $this->items[$i]->date );
				$feed .= '
								"publishedDate":"' . htmlspecialchars( $itemDate->rfc822() ) . '",
				';
			}
			// contentSnippet - A snippet (< 120 characters) version of the content attribute. The snippet does not contain any HTML tags.
			$item_description = $this->items[$i]->getDescription();
			...
			';
			$feed .= '
								"content":' . $this->validateJSONFeed( $item_description ) . '
			';
			$categories = $this->items[$i]->category;
			if ( $categories != NULL ) {
				$feed .= ',
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		# namespaces
		$feed .= " xmlns:atom=\"http://www.w3.org/2005/Atom\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:content=\"http://purl.org/rss/1.0/modules/content/\"";
		$feed .= ">\n";
		$feed .= "	<channel>\n";
		$feed .= "		<title>" . FeedCreator::iTrunc( htmlspecialchars( $this->title ), $this->truncSize ) . "</title>\n";
		...
		# atom:link
		$feed .= "		<atom:link href=\"" . htmlspecialchars( $this->syndicationURL, ENT_COMPAT, $this->encoding ) . "\" rel=\"self\" type=\"application/rss+xml\" />\n";
		if ( $this->image != null ) {
			$feed .= "		<image>\n";
			$feed .= "			<url>" . $this->image->url . "</url>\n";
			$feed .= "			<title>" . FeedCreator::iTrunc( htmlspecialchars( $this->image->title ), $this->truncSize ) . "</title>\n";
			$feed .= "			<link>" . $this->image->link . "</link>\n";
			if ( $this->image->width != "" ) {
		...
		if ( $this->copyright != "" ) {
			$feed .= "		<copyright>" . FeedCreator::iTrunc( htmlspecialchars( $this->copyright ), $this->truncSize ) . "</copyright>\n";
		}
		if ( $this->editor != "" ) {
			$feed .= "		<managingEditor>" . FeedCreator::iTrunc( htmlspecialchars( $this->editor ), $this->truncSize ) . "</managingEditor>\n";
		}
		if ( $this->webmaster != "" ) {
			$feed .= "		<webMaster>" . FeedCreator::iTrunc( htmlspecialchars( $this->webmaster ), $this->truncSize ) . "</webMaster>\n";
		}
		if ( $this->pubDate != "" ) {
		...
		if ( $this->docs != "" ) {
			$feed .= "		<docs>" . FeedCreator::iTrunc( htmlspecialchars( $this->docs ), $this->descriptionTruncSize ) . "</docs>\n";
		}
		if ( $this->ttl != "" ) {
			$feed .= "		<ttl>" . htmlspecialchars( $this->ttl ) . "</ttl>\n";
		}
		if ( $this->rating != "" ) {
			$feed .= "		<rating>" . FeedCreator::iTrunc( htmlspecialchars( $this->rating ), $this->descriptionTruncSize ) . "</rating>\n";
		}
		if ( $this->skipHours != "" ) {
		...
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			$feed .= "		<item>\n";
			$feed .= "			<title>" . FeedCreator::iTrunc( htmlspecialchars( strip_tags( $this->items[$i]->title ) ), $this->truncSize ) . "</title>\n";
			$feed .= "			<link>" . htmlspecialchars( $this->items[$i]->link ) . "</link>\n";
			...
			*/
			if ( $this->items[$i]->category != NULL ) {
				foreach ( $this->items[$i]->category AS $category ) {
					$feed .= "			<category>" . htmlspecialchars( $category ) . "</category>\n";
				}
			}
...
class AtomCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		if ( $this->copyright != "" ) {
			$feed .= "		<rights>" . FeedCreator::iTrunc( htmlspecialchars( $this->copyright ), $this->truncSize ) . "</rights>\n";
		}
		$feed .= "	<generator>" . $this->version() . "</generator>\n";
...
class MBOXCreator extends FeedCreator {
	...
	function createFeed() {
		for ( $i = 0; $i < count( $this->items ); $i ++ ) {
			...
			$feed .= "Subject: " . MBOXCreator::qp_enc( FeedCreator::iTrunc( $this->items[$i]->title, $this->truncSize ) ) . "\n";
			$feed .= "\n";
...
class HTMLCreator extends FeedCreator {
	...
	function createFeed() {
		...
		if ( $this->image != null ) {
			$imageStr = "<a href='" . $this->image->link . "'" . $targetInsert . ">" .
				...
				FeedCreator::iTrunc( htmlspecialchars( $this->image->title ), $this->truncSize ) .
		...
		if ( $this->title ) {
			$feedArray[] = "<div class='" . $this->stylePrefix . "title'><a href='" . $this->link . "' " . $targetInsert . " class='" . $this->stylePrefix . "title'>" .
				FeedCreator::iTrunc( htmlspecialchars( $this->title ), $this->truncSize ) . "</a></div>";
		}
			...
			if ( $this->items[$i]->title ) {
				if ( $this->items[$i]->link ) {
					$feedArray[] =
						...
						"item_title'" . $targetInsert . ">" . FeedCreator::iTrunc( htmlspecialchars( strip_tags( $this->items[$i]->title ) ), $this->truncSize ) .
						"</a></div>";
				} else {
					$feedArray[] =
						"<div class='" . $this->stylePrefix . "item_title'>" .
						FeedCreator::iTrunc( htmlspecialchars( strip_tags( $this->items[$i]->title ) ), $this->truncSize ) .
						"</div>";
				}
			}
	...
	function _generateFilename() {
		$fileInfo = pathinfo( $_SERVER["PHP_SELF"] );
		$len      = isset( $fileInfo["extension"] ) ? ( strlen( $fileInfo["extension"] ) + 1 ) : 0;

		return substr( $fileInfo["basename"], 0, - $len ) . ".html";
	}
}
...
/**
 * GoogleSiteMapIndex is a FeedCreator that implements Google Sitemap Index 0.9.
 ...
 * $rss->addItem($item);
 * //}
 * // valid format strings are: RSS091, RSS10, RSS20, PIE01, MBOX, OPML, ATOM03, HTML, JS
 * echo $rss->saveFeed("RSS091", "feed.xml");
 ***************************************************************************/