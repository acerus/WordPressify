...
class WPPB_Pagination{
	...
	function generate($total, $perPage = 10, $searchFor, $first, $prev, $next, $last, $currentPage){
		...
		$this->searchFor = ( ( $searchFor != '' ) ? '&searchFor='.$searchFor : '' );
	...
	function links(){
		...
		if (($this->pages) > 1){
			...
			if ($this->page != 1) {
				...
				$plinks[] = '<a href="?page=1' . esc_attr( $queryURL.$this->searchFor ) . '" id="pageLink" class="pageLink_fist">'.$this->first.'</a>';
				}
				$plinks[] = '&nbsp;<a href="?page=' . esc_attr( ($this->page - 1).$queryURL.$this->searchFor ) . '" id="pageLink" class="pageLink_previous">'.$this->prev.'</a>&nbsp;';
			}
